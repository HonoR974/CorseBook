...
export class PublicationPublicComponent implements OnInit {
  ...
  private getPublicationsPublic() {
    this.publicationService.getPublicationPublic().subscribe((data) => {
      ...
      this.publications.forEach(
        (element) => (element.isClickedComment = false)
      );
    });
  }
  ...
  publicationLiked(id: number) {
    this.publicationService.likePublication(id).subscribe((data) => {
  ...
  publicationDisliked(id: number) {
    this.publicationService.dislikedPublication(id).subscribe((data) => {
      this.publicationUpdate = data;
      this.getPublicationsPublic();
    });
  }
  ...
  createComment(id_publication: number) {
    ...
    this.commentService
      .createCommentByPublication(this.comment, id_publication)
      .subscribe((data) => {
  ...
  commentLiked(id: number) {
    this.commentService.commentLikedById(id).subscribe((data) => {});
  ...
  deletePublication(id: number) {
    Swal.fire({
      ...
      if (result.value) {
        Swal.fire('Deleted!', 'Votre publication a été supprimée.', 'success');
